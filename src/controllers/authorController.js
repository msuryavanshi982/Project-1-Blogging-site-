...
const createAuthor = async (req, res) => {
  try {
    ...
    if (!isValidName(fname)) {
      return res
        .status(400)
        .send({ status: false, msg: "fName should include Alphabates Only." });
    }
    if (!isValidName(lname)) {
      return res
        .status(400)
        .send({ status: false, msg: "lName should include Alphabates only." });
    }
    if (!isValidEmail(email)) {
      return res
        .status(400)
        .send({ status: false, msg: "provide a valid emailId" });
    }
    if (!isValidPassword(password)) {
      return res.status(400).send({
        status: false,
        msg: "Password must contain atleast 8 characters including one upperCase, lowerCase, special characters and Numbers",
      });
    }
    if (!["Mr", "Miss", "Mrs"].includes(title)) {
      return res.status(400).send({
        status: false,
        msg: "Title should only include 'Mr','Miss','Mrs'",
      });
    }
...
const loginAuthor = async (req, res) => {
  try {
    ...
    if (!checkCredentials) {
      return res
        .status(401)
        .send({ status: false, message: "Provide a valid credentials." });
    }
    let token = jwt.sign(
      {
